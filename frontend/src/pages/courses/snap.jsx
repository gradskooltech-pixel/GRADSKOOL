 ...
 *
 * Repositioned per request: leads with the free SNAP Sessions (live,
 * taught by ALP Sir, at /courses/snap/live) as the primary offering, with
 * the paid SNAP Mocks (₹1,499) kept as a secondary option on the same
...
const FAQS = [
  ...
  { q:'Are the SNAP Sessions actually free?', a:"Yes — every topic, taught live by ALP Sir, at no cost. Create a free account and start watching at /courses/snap/live." },
  ...
  { q:'Can I combine SNAP Mocks with NMAT Mocks?', a:'Yes. The SNAP + NMAT Mocks bundle is available at ₹2,499 — saving ₹500 compared to buying them separately.' },
]
  ...
  return (
    <>
      <PageSEO
        ...
        schema={[
          courseSchema({name:'SNAP Sessions',description:"Free SNAP Sessions taught live by ALP Sir, plus SNAP Mocks: 20 full-length mocks, 9 sectional tests, area-wise tests.",url:'/courses/snap',price:'0',mode:['online','live']}),
          faqSchema(FAQS),
        ]}
      />

      {/* hero — video left, text right */}
      <section style={{ display:'grid', gridTemplateColumns:'1fr 1.1fr', gap:0, alignItems:'center' }} className="snap-hero">
        <style>{`@media(max-width:960px){.snap-hero{grid-template-columns:1fr!important}}`}</style>
        <style>{S}</style>
        <div style={{ padding:'40px' }}>
          <CloudinaryVideo url={VIDEO_URL} title="SNAP — a session with ALP Sir" />
        </div>

        <div style={{ padding:'40px 48px 40px 8px' }}>
          <div className="eyebrow" style={{ marginBottom:14 }}><span className="dot" />SNAP · Free Sessions · Mocks · Sectionals</div>
          <h1 className="d-xl" style={{ marginBottom:20 }}>SNAP, <em style={R}>taught live.</em></h1>
          ...
          <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
            <Link href="/courses/snap/live" className="btn btn-red">Enrol Now — Free →</Link>
            <a href="https://wa.me/917838737388?text=Hi%20ALP%20Sir%2C%20I%20want%20to%20know%20about%20SNAP" target="_blank" rel="noopener noreferrer" className="btn btn-wa">
          ...
          <div style={{ display:'flex', gap:28, marginTop:44, paddingTop:24, borderTop:'var(--border)', flexWrap:'wrap' }}>
            {[['Free','Live sessions'],['20','Full-length mocks'],['9','Sectional tests'],['15+','Symbiosis colleges']].map(([v,l]) => (
              <div key={l}>
        ...
        <div className="container">
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:20 }} className="enrol-grid-2">
            ...
            <div style={{ background:'#fff', border:`2px solid var(--red)`, borderRadius:6, padding:'32px 36px', position:'relative' }}>
              ...
              <div style={{ fontFamily:'var(--font-serif)', fontSize:22, color:'var(--black)', marginTop:8, marginBottom:6 }}>SNAP Sessions</div>
              ...
              <Link href="/courses/snap/live" className="btn btn-red" style={{ width:'100%', justifyContent:'center' }}>Enrol Now — Free →</Link>
            </div>
            ...
            <div style={{ background:'#fff', border:'var(--border)', borderRadius:6, padding:'32px 36px' }}>
              ...
              {['20 full-length SNAP mocks — real exam interface','9 sectional tests','Area-wise topic tests','Detailed analysis report'].map(item => (
        ...
        <div className="container">
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:16 }} className="also-grid-2">
            ...
            <Link href="/courses/cat/cathlete" style={{ background:'var(--off)', border:'var(--border)', borderRadius:4, padding:'24px', textDecoration:'none', display:'block' }}>
              ...
              <p style={{ fontFamily:'var(--font-body)', fontSize:13, color:'var(--g700)', lineHeight:1.6, marginBottom:12 }}>GRADSKOOL's intensive CAT crash course. A lot of SNAP students take both.</p>
      ...
      <section style={{ padding:'72px 0', borderBottom:'var(--border)', background:'var(--off)' }}>
        <div className="container">
          <div style={{ marginBottom:32 }}>
            ...
            <p style={{ fontFamily:'var(--font-body)', fontSize:14, color:'var(--g700)', marginTop:10, maxWidth:520, lineHeight:1.8 }}>Language Skills, Quantitative-Data Interpretation, and Analytical & Logical Reasoning — all three sections in equal depth.</p>
          </div>
      ...
      <section style={{ padding:'72px 0', textAlign:'center' }}>
        <div className="container">
          ...
          <p style={{ fontFamily:'var(--font-body)', fontSize:14, color:'var(--g700)', lineHeight:1.8, maxWidth:380, margin:'0 auto 32px' }}>
            No payment, no risk — every SNAP topic taught live by ALP Sir. Add mocks whenever you're ready to test yourself.
          </p>
          <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
            <Link href="/courses/snap/live" className="btn btn-red">Enrol Now — Free →</Link>
            <a href="https://wa.me/917838737388?text=Hi%20ALP%20Sir%2C%20about%20SNAP" target="_blank" rel="noopener noreferrer" className="btn btn-wa"><span className="wa-dot" />WhatsApp first</a>
          </div>
        </div>
      </section>
      ...
      <div className="mob-sticky">
        <div>
          <div style={{ fontFamily:'var(--font-sans)', fontSize:10, fontWeight:600, letterSpacing:'.07em', textTransform:'uppercase', color:'var(--g500)' }}>SNAP Sessions</div>
        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          <Link href="/courses/snap/live" style={{ fontFamily:'var(--font-sans)', fontSize:12, fontWeight:600, background:'var(--red)', color:'#fff', padding:'8px 18px', borderRadius:2, textDecoration:'none' }}>Enrol →</Link>
        </div>
      </div>
      <WaFloat msg="Hi ALP Sir, I want to know more about SNAP" />
    </>
  )
}