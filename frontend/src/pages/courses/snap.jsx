 ...
 * pricing card, as used by CAThlete/CATalysis/Books/ALPgebra) instead of
 * its earlier dark single-column hero — for visual consistency across
 * every course page.
 *
...
export default function SNAPPage() {
  return (
    <>
      ...
      {/* hero */}
      <section style={{ display:'grid', gridTemplateColumns:'1.2fr 1fr' }} className="snap-hero">
        <style>{`@media(max-width:960px){.snap-hero{grid-template-columns:1fr!important}}`}</style>
        <style>{S}</style>
        <div style={{ padding:'72px 48px 56px' }}>
          <div className="eyebrow" style={{ marginBottom:14 }}><span className="dot" />SNAP · Mocks · Sectionals · Area-wise Tests</div>
          <h1 className="d-xl" style={{ marginBottom:20, maxWidth:520 }}>SNAP <em style={R}>Mocks.</em></h1>
          ...
          <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
            ...
            <a href="https://wa.me/917838737388?text=Hi%20ALP%20Sir%2C%20I%20want%20to%20know%20about%20SNAP%20Mocks" target="_blank" rel="noopener noreferrer" className="btn btn-wa">
        ...
        <div style={{ background:'var(--off)', display:'flex', flexDirection:'column', justifyContent:'center', padding:'40px 48px', gap:16 }}>
          <div style={{ background:'#fff', border:'var(--border)', borderRadius:4, padding:'28px 32px' }}>
            ...
            {['20 full-length SNAP mocks — real exam interface and timing','9 sectional tests — Language, Quant, Reasoning individually','Area-wise topic tests for granular practice','Detailed analysis report after every attempt','Percentile estimate and topic-wise weakness tracking'].map(item => (
          ...
          <div style={{ background:'#fff', border:'var(--border)', borderRadius:4, padding:'20px 24px' }}>
            <div style={{ fontFamily:'var(--font-serif)', fontSize:15, color:'var(--black)', marginBottom:6 }}>SNAP — Complete Course, Free</div>
            ...
            <Link href="/courses/snap/live" className="btn btn-outline" style={{ fontSize:12, padding:'9px 18px' }}>Explore free course →</Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding:'72px 0', textAlign:'center' }}>
        <div className="container">
          <h2 style={{ fontFamily:'var(--font-serif)', fontSize:'clamp(26px,4vw,44px)', fontWeight:400, color:'var(--black)', marginBottom:14 }}>
            Ready to test yourself? <em style={R}>Start today.</em>
          </h2>
